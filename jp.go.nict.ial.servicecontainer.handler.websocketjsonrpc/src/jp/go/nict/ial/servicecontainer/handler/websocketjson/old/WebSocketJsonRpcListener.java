...
@WebSocket
public class WebSocketJsonRpcListener {
	@OnWebSocketConnect
	public void onConnect(Session session) {
		...
		ServiceContext sc = WebSocketJsonRpcServlet.getStaticServiceContext();
		ClassLoader cl = Thread.currentThread().getContextClassLoader();
		ServiceFactory factory = new ServiceLoader(sc).loadServiceFactory(cl, serviceName);
		service = factory.createService(cl, sc, factory.getInterfaces().iterator().next());
	...
	public void onText(String message) {
		final JsonRpcRequest req = JSON.decode(message, JsonRpcRequest.class);
		if(req.getHeaders() == null){
			req.setHeaders(new RpcHeader[]{});
		}
		ServiceContext sc = new RpcServiceContext(WebSocketJsonRpcServlet.getStaticServiceContext(), req.getHeaders());
		Method method = null;
		...
		try{
			...
			if(method == null){
				logger.warning(String.format(
						...
						, serviceName));
				session.getRemote().sendString(create404(req.getId()));
				return;
			}
			Object result = null;
				...
				if(service instanceof StreamingNotifier<?>){
					((StreamingNotifier<Object>) service).setReceiver(new StreamingReceiver<Object>() {
						...
						public boolean receive(Object result) {
							...
							r.setResult(result);
							try{
								session.getRemote().sendString(JSON.encode(r));
							} catch(Exception e){
...
//				MimeHeadersUtil.setToHttpServletResponse(resMimeHeaders, response);
		...
		try{
			session.getRemote().sendString(JSON.encode(res));
		} catch(IOException e){
	...
	private static Logger logger = Logger.getLogger(WebSocketJsonRpcListener.class.getName());
}
